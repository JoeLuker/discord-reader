...
def main():
    ...
    with col1:
        fig = px.bar(df_servers.head(10), x='Server', y='Messages', 
                     title="Top 10 Most Active Servers")
        st.plotly_chart(fig, use_container_width=True)
    
    with col2:
        fig = px.pie(df_servers.head(10), values='Messages', names='Server',
    ...
    if selected_server != "All":
        ...
        server_info = next(s for s in data['servers'].values() if s['name'] == selected_server)
