...
def main():
    ...
    if not data:
        st.error(
            "No analysis data found. Run discord_reader.py or discord_reader_oauth.py first!"
        )
        return
    ...
    if data_type == "regular" and any(
        "channels" in s for s in data["servers"].values()
    ):
        with col2:
            total_channels = sum(
                len(s.get("channels", [])) for s in data["servers"].values()
            )
    ...
    for server_id, server in data["servers"].items():
        if data_type == "regular" and "channels" in server:
            ...
            active_channels = len(
                [c for c in server["channels"] if c.get("message_count", 0) > 0]
            )
    ...
    if data_type == "oauth":
        ...
        for _, row in df_servers.iterrows():
            with st.expander(f"**{row['Server']}**" + (" 👑" if row["Owner"] else "")):
                ...
                server_info = next(
                    s for s in data["servers"].values() if s["name"] == row["Server"]
                )
        ...
        with col1:
            fig = px.bar(
                df_servers.sort_values("Messages", ascending=False).head(10),
                x="Server",
                y="Messages",
        ...
        with col2:
            fig = px.pie(
                df_servers[df_servers["Messages"] > 0].head(10),
                values="Messages",
                names="Server",
