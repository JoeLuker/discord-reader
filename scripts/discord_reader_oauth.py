...
class DiscordAnalyzerOAuth:
    ...
    async def fetch_user_data(self):
        ...
        async with aiohttp.ClientSession() as session:
            # Get user info
            async with session.get(f'{self.api_base}/users/@me', headers=headers) as resp:
                if resp.status == 200:
            ...
            # Get guilds
            async with session.get(f'{self.api_base}/users/@me/guilds', headers=headers) as resp:
                if resp.status == 200:
    ...
    async def analyze_guild(self, session, guild_id, headers):
        ...
        try:
            async with session.get(f'{self.api_base}/guilds/{guild_id}/channels', headers=headers) as resp:
                if resp.status == 200:
                    ...
                    for channel in text_channels[:5]:  # Limit to 5 channels per guild for demo
                        channel_data = {
                            'id': channel['id'],
                            'name': channel['name'],
                            'type': channel['type']
                        ...
                        await self.analyze_channel_messages(session, channel['id'], headers)
                    ...
                    for msg in messages:
                        if msg['author']['id'] == self.user_data['id']:
                            ...
                            if msg.get('referenced_message'):
                                replied_to = msg['referenced_message']['author']['username']
