...
class MechInterpAnalyzer:
    ...
    async def fetch_server_info(self, session):
        ...
        async with session.get(
            f"{self.api_base}/guilds/{self.mech_interp_id}", headers=self.headers
        ) as resp:
            if resp.status == 200:
    ...
    async def fetch_channels(self, session):
        async with session.get(
            f"{self.api_base}/guilds/{self.mech_interp_id}/channels",
    ...
    async def fetch_channel_messages(
        self, session, channel_id, channel_name, limit=500
    ):
                    ...
                    if resp.status == 200:
                        ...
                        for message in messages:
                            ...
                            message_data = {
                                ...
                                "reply_to": (
                                    message.get("referenced_message", {}).get("id")
                                    if message.get("referenced_message")
                                    else None
                                ),
                                "thread_id": (
                                    message.get("thread", {}).get("id")
                                    if message.get("thread")
                                    else None
                                ),
                            }

                            self.data["channels"][channel_id]["messages"].append(
                                message_data
                            )
                            ...
                            if not self.data["channels"][channel_id][
                                "first_message_time"
                            ]:
                                self.data["channels"][channel_id][
                                    "first_message_time"
                                ] = message["timestamp"]
                            self.data["channels"][channel_id]["last_message_time"] = (
                                message["timestamp"]
                            )
                        ...
                        print(
                            f"  Fetched {len(messages)} messages (total: {messages_fetched})"
                        )
                        ...
                        print(
                            f"  Error fetching messages from #{channel_name}: {resp.status}"
                        )
                        break
        ...
        print(
            f"  Completed #{channel_name}: {messages_fetched} messages, {len(users_in_channel)} users"
        )
    ...
    async def analyze(self):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            for channel_id, channel_data in self.data["channels"].items():
                await self.fetch_channel_messages(
                    session, channel_id, channel_data["name"]
                )
            ...
            active_channels = len(
                [c for c in self.data["channels"].values() if c["message_count"] > 0]
            )
            ...
            print(
                f"- Most active channel: {max(self.data['channel_activity'], key=self.data['channel_activity'].get) if self.data['channel_activity'] else 'None'}"
            )
            ...
            if self.data["channel_activity"]:
                ...
                sorted_channels = sorted(
                    self.data["channel_activity"].items(),
                    key=lambda x: x[1],
                    reverse=True,
                )
