...
class DiscordUserAnalyzer:
    ...
    async def fetch_guilds(self, session):
        async with session.get(f'{self.api_base}/users/@me/guilds', headers=self.headers) as resp:
            if resp.status == 200:
    ...
    async def analyze_guild(self, session, guild_id):
        try:
            # Get guild channels
            async with session.get(f'{self.api_base}/guilds/{guild_id}/channels', headers=self.headers) as resp:
                if resp.status == 200:
                    channels = await resp.json()
                    text_channels = [c for c in channels if c['type'] == 0]  # Text channels only
                    
                    print(f"Analyzing {len(text_channels)} channels in {self.data['servers'][guild_id]['name']}")
                    
                    for channel in text_channels[:10]:  # Limit to 10 channels per server
                        channel_data = {
                            'id': channel['id'],
                            'name': channel['name'],
                            'type': channel['type'],
                        ...
                        await self.analyze_channel_messages(session, channel['id'], channel_data)
                        
                        self.data['servers'][guild_id]['channels'].append(channel_data)
                        
                elif resp.status == 403:
                    print(f"No permission to access channels in {self.data['servers'][guild_id]['name']}")
                else:
    ...
    async def analyze_channel_messages(self, session, channel_id, channel_data):
        try:
            ...
            async with session.get(f'{self.api_base}/channels/{channel_id}/messages?limit=100', headers=self.headers) as resp:
                if resp.status == 200:
                    ...
                    for message in messages:
                        if message['author']['id'] == self.user_id:
                            ...
                            if message.get('referenced_message'):
                                replied_to = message['referenced_message']['author']['username']
                    ...
                    print(f"Failed to get messages for channel {channel_id}: {resp.status}")
    ...
    async def analyze(self):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            print(f"- Most active hours: {sorted(self.data['activity_times'].items(), key=lambda x: x[1], reverse=True)[:3]}")
            print(f"- Top interactions: {sorted(self.data['interactions'].items(), key=lambda x: x[1], reverse=True)[:5]}")
